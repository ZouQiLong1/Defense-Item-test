...
//        Manager manager1 = objectMapper.readValue(manager, Manager.class);
        ...
        boolean flag = updateUserService.updateUser(user);
...
//            request.setAttribute("manager",manager1);
            response.sendRedirect(request.getContextPath() + "/indexInfoByPageServlet");//            response.sendRedirect(request.getContextPath() + "/indexInfoServlet");
        } else {
